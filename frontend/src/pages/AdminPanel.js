...
const AdminPanel = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-stone-950 via-stone-900 to-stone-950 pb-24">
      ...
      <Dialog open={showGlobalAnnouncementDialog} onOpenChange={setShowGlobalAnnouncementDialog}>
        <DialogContent className="bg-stone-900 border-stone-700 text-white max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-orange-400 flex items-center gap-2">
              <Megaphone className="w-5 h-5" />
              Crear Anuncio Global (1000 Lps)
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="bg-orange-900/20 rounded-lg p-3 border border-orange-500/20">
              <p className="text-orange-300 text-sm">
                Este anuncio aparecerá para TODOS los usuarios de la app
              </p>
            </div>
            
            <div>
              <Label className="text-stone-300">Título *</Label>
              <Input
                value={globalAnnTitle}
                onChange={(e) => setGlobalAnnTitle(e.target.value)}
                placeholder="Ej: ¡Gran Oferta de Temporada!"
                className="bg-stone-800 border-stone-700 text-white mt-1"
              />
            </div>
            
            <div>
              <Label className="text-stone-300">Contenido *</Label>
              <Textarea
                value={globalAnnContent}
                onChange={(e) => setGlobalAnnContent(e.target.value)}
                placeholder="Describe la oferta o anuncio..."
                className="bg-stone-800 border-stone-700 text-white placeholder:text-stone-500 mt-1"
                rows={3}
              />
            </div>
            
            <div>
              <Label className="text-stone-300">URL de Imagen (opcional)</Label>
              <Input
                value={globalAnnImageUrl}
                onChange={(e) => setGlobalAnnImageUrl(e.target.value)}
                placeholder="https://..."
                className="bg-stone-800 border-stone-700 text-white mt-1"
              />
            </div>
            ...
            <div>
              <Label className="text-stone-300">URL de Enlace (opcional)</Label>
              <Input
                value={globalAnnLinkUrl}
                onChange={(e) => setGlobalAnnLinkUrl(e.target.value)}
                placeholder="https://..."
                className="bg-stone-800 border-stone-700 text-white mt-1"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label className="text-stone-300">Prioridad</Label>
                <select
                  value={globalAnnPriority}
                  onChange={(e) => setGlobalAnnPriority(parseInt(e.target.value))}
                  className="w-full bg-stone-800 border border-stone-700 text-white rounded-lg p-2 mt-1"
                >
                  <option value={0}>Normal</option>
                  <option value={5}>Medio</option>
                  <option value={10}>Alto (Destacado)</option>
                </select>
              </div>
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={globalAnnExpiresDays}
                  onChange={(e) => setGlobalAnnExpiresDays(parseInt(e.target.value) || 30)}
                  className="bg-stone-800 border-stone-700 text-white mt-1"
                  min={1}
                  max={365}
                />
            ...
            <Button 
              onClick={handleCreateGlobalAnnouncement} 
